        ...
        if action == 'to_ordered':
            ...
            if detail_number_check == 'on' and description_number_check == 'on':
                ...
                for i in info:
                    ...
                    i.update({'id':str(id)})
                    data.append(i)
            return render(request,'tip.html',{'data':data})
    ...
    if tri:
        for a in que:
            ...
            for i in info:
               ...
               i.update({'id':str(id)})
               data.append(i)
    return render(request,'thrust.html',{'data':data})
    ...
    if tri:
        for a in que:
            ...
            for i in info:
               ...
               i.update({'id':str(id)})
               data.append(i)
    return render(request,'steel_wheel.html',{'data':data})
    ...
    if tri:
        for a in que:
            ...
            for i in info:
               ...
               i.update({'id':str(id)})
               data.append(i)
    return render(request,'bearing.html',{'data':data})
...
def brakepads(request):
    ...
    if request.method == 'POST': 
        id_post = request.POST.getlist('id')
        amount = request.POST.getlist('amount')
        if len(id_post) == 1 and len(amount) == 1:
            id_post = int(id_post[0])
            amount = int(amount[0])
        obj = model.objects.get(id=id_post)
        obj_amount = obj.amount_available
    ...
    if tri:
        for a in que:
            ...
            for i in info:
               ...
               i.update({'id':str(id)})
               data.append(i)
    return render(request,'brakepads.html',{'data':data})
